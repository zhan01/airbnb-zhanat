class RoomsController < ApplicationController
  before_action :set_room, only: [:show, :edit, :update, :search]


  def index
    Room.reindex
    @rooms = params[:query] ? Room.search(params[:query]) : Room.all
  end
  ...
  def room_params
    params.require(:room).permit(
        ...
        :is_air, :is_heating, :is_internet, :price, :active, {photos: []}
    )
  end
end
